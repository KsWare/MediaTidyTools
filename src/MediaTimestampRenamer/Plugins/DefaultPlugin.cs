...
namespace KsWare.MediaTimestampRenamer.Plugins {
	...
	internal class DefaultPlugin : IProcessPlugin {
		...
		internal static readonly List<string> RawDataFormats = LoadRawDataFormats();
		...
		public bool IsMatch(FileInfo fileInfo, out Match match) {
			match = null;
			if (FileUtils.IsMovie(fileInfo.Name)) return false;
		...
		public bool Process(MediaFileInfo fileInfo) {
			...
			if (!fileInfo.Rename(pluginName:Name)) return false;

			// Rename dependencies 
			fileInfo.RenameDependency(".thumbs", ".256");
			fileInfo.RenameDependency(".preview", "_~AuroraPreviewAuto");
			fileInfo.RenameDependencyRawExtension(RawDataFormats, "RAW", true, pluginName:Name);
			fileInfo.RenameDependencyExtension(".AUH", "AUH", true, pluginName:Name, out _); // Aurora HDR
			return true;
		}
